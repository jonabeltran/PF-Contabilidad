...
public class GenerarPDF
{
    ...
    String hasta=datMayor.jFormattedTextField2.getText();
    double saldo=0;
    //Metodo principal del ejemplo
    public GenerarPDF(String titulo, String nomPDF)
    {
        strRotuloPDF = titulo;
    ...
    private void agregarTabla(Paragraph parrafo) throws BadElementException 
    {
         bd=new coneccionBD();
        try{
            String sql="select codigo_cta,descripcion,nivel from plan_cuentas";
            Statement miSt=bd.dameSentencia();
            ResultSet miRt=miSt.executeQuery(sql);
            while(miRt.next()){
        //Anchos de las columnas
      //  float anchosFilas[] = { 0.2f,1f,1f,0.5f,0.5f,0.3f,1f,1.5f,0.7f };
      float anchosFilas[] = { 0.3f,0.5f,0.3f,0.5f,0.5f,0.5f,0.3f,0.5f,0.5f };
        PdfPTable tabla = new PdfPTable(anchosFilas);
        String rotulosColumnas[] = {"Asiento","Fech. Cont.","Renglon","Leyenda","Suc","Sec","Debe/Haber","Importe","Saldo"};
        // Porcentaje que ocupa a lo ancho de la pagina del PDF
        tabla.setWidthPercentage(100);
        //Alineacion horizontal centrada
        tabla.setHorizontalAlignment(Element.ALIGN_CENTER);
        //agregar celda que ocupa las 9 columnas de los rotulos
        PdfPCell cell = new PdfPCell(new Paragraph("Codigo de Cta: "+miRt.getString("codigo_cta")+"   Descripcion: "+miRt.getString("descripcion")+"  Nivel: "+miRt.getString("nivel")));
        cell.setColspan(9);
        ...
        try{
               ...
               String strConsultaSQL = "SELECT * FROM diario_mayor where nro_cuenta='"+miRt.getString("codigo_cta")+"' and (fecha_contable BETWEEN '"+desde+"' and '"+hasta+"')";
            ...
            while (rs.next())
            {           //Agregar 9 celdas
                    cell = new PdfPCell(new Paragraph(rs.getString("nro_asiento"),fuente8 ));
                    tabla.addCell(cell);
                    ...
                    tabla.addCell(cell);
                    cell = new PdfPCell(new Paragraph(rs.getString("sucursal"),fuente8));
                    tabla.addCell(cell);
                    cell = new PdfPCell(new Paragraph(rs.getString("seccion"),fuente8) );
                    tabla.addCell(cell);
                     //calculos
                    if(rs.getString("debe_haber").equals("0")){
                        d_h="Debe";
                        saldo=saldo+Double.parseDouble(rs.getString("importe"));
                    }else{
                        d_h="Haber";
                        saldo=saldo-Double.parseDouble(rs.getString("importe"));
                    }
                    //--------------------------------------
                    cell = new PdfPCell(new Paragraph(d_h,fuente8));
                    tabla.addCell(cell);
                    cell = new PdfPCell(new Paragraph(rs.getString("importe"),fuente8));
                    tabla.addCell(cell);
                    cell = new PdfPCell(new Paragraph(String.valueOf(saldo),fuente8 ) );
                    tabla.addCell(cell);
            }
        ...
        cell = new PdfPCell(new Paragraph("Total: "+String.valueOf(saldo)));
        cell.setColspan(9);
        //Centrar contenido de celda
